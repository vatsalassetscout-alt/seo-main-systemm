...
export async function fetchProjectsFromSheet(
  ...
  try {
    if (rows.length === 0) {
      ...
      await initSheetHeaders(cleanId, sheetName || 'sheet1', ['User ID', 'Users', 'Project Name', 'Domain', 'Location', 'Region', 'Keyword1', 'Keyword2', 'Keyword3', 'Keyword4', 'Keyword5', 'Keyword6', 'Keyword7', 'Keyword8'], token);
      return [];
    }

    const headers = rows[0] || [];

    // Collision-safe column detection (shared with the server-side mapper,
    // so the sheet is read the exact same way everywhere in the app).
